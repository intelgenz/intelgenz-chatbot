...
def _missing_deepseek_settings() -> list[str]:
...
def get_llm_with_tools():
    ...
    if llm_with_tools is None:
        missing_settings = _missing_deepseek_settings()
        if missing_settings:
            missing = ", ".join(missing_settings)
            raise RuntimeError(f"Missing required DeepSeek environment variables: {missing}")
